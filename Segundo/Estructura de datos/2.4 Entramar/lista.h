...
public:
	...
	void display(std::ostream& out) const {
		out << "[";
		if (this->_prim->_sig != this->_prim) {
			out << this->_prim->_sig->_elem;
			Nodo* current = _prim->_sig->_sig;
			while (current != _prim) {
				out << ", " << current->_elem;
				current = current->_sig;
			}
		}
		out << "]";
		out << "\n";
	...
	//EJERCICIO:
	void attach(Nodo* nodo, Nodo* despues) {
		despues->_ant->_sig = nodo;
		nodo->_ant = despues->_ant;
		despues->_ant = nodo;
		nodo->_sig = despues;
	}
	...
	void otroAttach(Nodo* nodo, Nodo* antes) {
		nodo->_sig = antes->_sig;
		antes->_sig->_ant = nodo;
		antes->_sig = nodo;
		nodo->_ant = antes;
	}

	void entramar(Lista& other) {
		if (other.esVacia()) return;

		Nodo* current = this->_prim;
		Nodo* currentOther = other._prim;

		while (current != nullptr && currentOther != nullptr) {
			Nodo* aux = current->_sig;
			Nodo* aux2 = currentOther->_sig;
			//otroAttach(currentOther, current);
			other.dettach(currentOther);	
			attach(currentOther, current->_sig);
			currentOther = aux2;
			current = aux;
		}
		...
		while (currentOther != nullptr) {
			Nodo* aux3 = currentOther->_sig;
			other.dettach(currentOther);
			attach(currentOther, this->_ult);
			currentOther = currentOther->_sig;
		}
	}
