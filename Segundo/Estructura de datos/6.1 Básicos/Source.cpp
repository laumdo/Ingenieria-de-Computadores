...
/*COSTE:
* 1 Para el caso base, asumimos que el árbol es degenerado (caso peor)
*   T(n)= c0 si n=0
* 2 Para lo que no es caso base: asumimos que es degenerado (no podemos garantizar que está equilibrado)
*   T(n) = T(n-1) + c1 + c2 //c1 es del T(0), c2 es de que las operaciones recursivas que hay que hacer son constantes, podemos dejarlas como una cte:
*   T(n) = T(n-1) + c1' si n>0
* Si aplicamos el teorema de la resta (FAL):
*   a=1; b=1; d=0 --> a=1 : O(n^(0+1)) = O(n), es lineal
* 
* Si asumimos que es balanceado:
* T(n) = c0 si n=0
* T(n) = T(n/2) + T(n/2) + c1 si n>0 --> T(n) = 2T(n/2) + c1
* Aplicando el terorema de la división:
*  a=2; b=2; d=0 --> 2 > 2^0=1 : O(n^(log_2_2)) = O(n^1) = O(n), es lineal
* Paso una sóla vez por cada nodo, es lineal.
*/
template <typename T> int num_nodos(const BinTree<T>& tree) {
    if (tree.empty()) return 0; //arbol vacío
    //else if (tree.left().empty() && tree.right().empty()) return 1; //es hoja (no es necesario)
    else {
        int num_iz = num_nodos(tree.left());
        int num_der = num_nodos(tree.right());
        int num_nodos_tree = num_iz + num_der + 1;
        return num_nodos_tree;
    }
}

template <typename T> int num_hojas(const BinTree<T>& tree) {
    if (tree.empty()) return 0; //arbol vacío
    else if (tree.left().empty() && tree.right().empty()) return 1; //es hoja
    else {
        int num_iz = num_hojas(tree.left());
        int num_der = num_hojas(tree.right());
        int num_hojas_tree = num_iz + num_der;
        return num_hojas_tree;
    }
}
...
template <typename T> int altura(const BinTree<T>& tree) {
    ...
    //else if (tree.left().empty() && tree.right().empty()) return 1; //es hoja (no es necesario)
        ...
        int altura = max(altura_iz, altura_der)+1;
        return altura;
    }
}
...
void resuelveCaso() {
    ...
    cout << num_nodos(tree)<< " " << num_hojas(tree) << " " << altura(tree);
    cout << endl;
