...
tZurdo zurdo(BinTree<char> tree) {
    if (tree.empty()) {
        return { true,0 };
    }
        ...
        auto [esZurdo_iz, total_iz] = zurdo(tree.left());
        auto [esZurdo_der, total_der] = zurdo(tree.right());
        tZurdo result;
        result.total = total_iz + total_der + 1;
        result.esZurdo = esZurdo_iz && esZurdo_der && (total_iz > total_der);
        return result;
    }
}
...
void resuelveCaso() {
    ...
    int total = 1, iz = 0;
    tZurdo sol = zurdo(tree);
    if (sol.esZurdo) cout << "SI";
    else cout << "NO";
