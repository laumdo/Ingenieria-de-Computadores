...
rescate resolver(BinTree<int> tree) {
    if (tree.empty()) return { 0,0 };
        ...
        rescate iz = resolver(tree.left());
        rescate der = resolver(tree.right());
        rescate result;
        if (tree.root() != 0 && (iz.grupos==0 && der.grupos==0)) result.grupos = 1;
        else result.grupos = iz.grupos + der.grupos;
        result.rescatados = max(iz.rescatados + tree.root(), der.rescatados + tree.root());
        return result;
    }
}
...
void resuelveCaso() {
    ...
    rescate sol = resolver(tree);
    cout << sol.grupos << " " << sol.rescatados << endl;
}
