...
completo resolver(BinTree<char> tree) {
    if (tree.empty()) return { 0,true};
    else if (tree.left().empty() && tree.right().empty()) {
        return { 1,true };
    }
    else {
        completo iz = resolver(tree.left());
        completo der = resolver(tree.right());
        completo result;
        result.h = max(iz.h, der.h) + 1;
        result.esCompleto = (iz.h == der.h) && iz.esCompleto && der.esCompleto;
        return result;
    }
}
...
void resuelveCaso() {
    ...
    completo sol = resolver(tree);
    if (sol.esCompleto) cout << "SI" << endl;
    else cout << "NO" << endl;
}
