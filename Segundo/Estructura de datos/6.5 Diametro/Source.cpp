...
barrera area(BinTree<char> arbol) {
    if (arbol.empty()) return{ false,0 };
        ...
        barrera areaIz = area(arbol.left());
        barrera areaDer = area(arbol.right());
        barrera sol;
        if (arbol.root() == 'V') {
            sol.esBarrera = false;
            if (areaIz.esBarrera && areaDer.esBarrera) {
                if (max(areaIz.suma, areaDer.suma) > 1) sol.suma = max(areaIz.suma, areaDer.suma);
                else sol.suma = 1;
            }
            else if (areaIz.esBarrera) sol.suma = areaDer.suma + 1;
            else if (areaDer.esBarrera)sol.suma = areaIz.suma + 1;
            else sol.suma = areaIz.suma + areaDer.suma + 1;
        }
        else {
            sol.esBarrera = true;
            sol.suma = max(areaIz.suma, areaDer.suma);
        }
        return sol;
    }
}
