...
public class Main {
	...
	private static Options build_options() {
		...
		cmdLineOptions.addOption(Option.builder("m").longOpt("mode").hasArg()
				.desc("Execution Mode. Possible values: 'batch', 'gui' ").build());
	...
	private static void parse_in_file_option(CommandLine line) throws ParseException {
		...
		if (_mode.equals("batch") && _in_file == null) {
	...
	private static void parse_mode_option(CommandLine line) throws ParseException {
		...
		_mode = s;
		if (!s.equalsIgnoreCase("batch") && !s.equalsIgnoreCase("gui"))
	...
	private static void start_GUI_mode() throws Exception {
		...
		c.load_data(j);
			...
			Simulator s = new Simulator(20, 15, 600, 800,
					animalsFactory, regionsFactory);
			c = new Controller(s);
		}
