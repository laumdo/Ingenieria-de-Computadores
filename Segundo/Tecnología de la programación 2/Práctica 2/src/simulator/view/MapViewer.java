...
public class MapViewer extends AbstractMapViewer {
		...
		addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				...
				case 's':
					...
					if (_currState != null) {
						int next = _currState.ordinal() + 1;
						if (next >= State.values().length) {
							_currState = null;
						} else {
							_currState = State.values()[next];
						}
					} else {
	...
	protected void paintComponent(Graphics g) {
		...
		if (_showHelp) {
			...
			g.drawString("s: show animals of a specific state", 10, 30);
		}
	}
	...
	private boolean visible(AnimalInfo a) {
		...
		return (a.get_state().equals(_currState) || _currState == null);
	}

	private void drawObjects(Graphics2D g, Collection<AnimalInfo> animals, Double time) {
		// TODO Dibujar el grid de regiones
		int tamw = _width / _cols;
		int tamh = _height / _rows;
		_objs = animals;
		_time = time;
		...
		for (AnimalInfo a : animals) {
			...
			if (esp_info == null) {
				Color c = ViewUtils.get_color(a.get_genetic_code());
				esp_info = new SpeciesInfo(c);
		...
		for (Entry<String, SpeciesInfo> e : _kindsInfo.entrySet()) {
			SpeciesInfo spInf = e.getValue();
			g.setColor(spInf._color);
			drawStringWithRect(g, 10, pos, e.getKey() + ": " + spInf._count);
			pos += 20;
			spInf._count = 0;
		}
	}
